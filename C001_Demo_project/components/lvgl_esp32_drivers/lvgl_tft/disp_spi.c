...
/* DMA Transactions to reserve before queueing additional DMA transactions. A 1/10th seems to be a good balance. Too many (or all) and it will increase latency. 
   在其他DMA事务排队之前要保留的DMA事务。1/10似乎是一个很好的平衡。太多(或全部)将增加延迟。*/
...
/**
  * @brief  在SPI总线上分配设备。
  * @param  host：SPI外设以在其上分配设备
  * @param  devcfg：设备的SPI接口协议配置
  * @retval void
  */
...
/**
  * @brief  SPI添加设备初始化并设置速度
  * @param  host：SPI外设以在其上分配设备
  * @param  clock_speed_hz：传输速度
  * @retval void
  */
void disp_spi_add_device_with_speed(spi_host_device_t host, int clock_speed_hz)
{
    ...
    spi_device_interface_config_t devcfg={
        ...
        /* dummy bits should be explicitly handled via DISP_SPI_VARIABLE_DUMMY as needed 
           根据需要，应该通过dis_spi_variable_dummy显式地处理虚位*/
	...
	/* create the transaction pool and fill it with ptrs to spi_transaction_ext_t to reuse
       创建事务池并使用ptrs填充它，以便spi_transaction_ext_t重用 */
