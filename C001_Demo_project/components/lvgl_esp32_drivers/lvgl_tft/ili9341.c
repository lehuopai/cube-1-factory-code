 ...
 *LCD需要一堆命令/参数值进行初始化。它们存储在这个结构体中。*/
typedef struct {
    uint8_t cmd;
    uint8_t data[16];
...
/**
  * @brief  ili9341 spi 发送数据
  * @param  data：数据指针
  * @param  length：数据长度
  * @retval void
  */
static inline void ili9341_send_data(void * data, uint16_t length)
{
    disp_wait_for_pending_transactions();
...
/**
  * @brief  ili9341 spi 发送颜色数据
  * @param  data：数据指针
  * @param  length：数据长度
  * @retval void
  */
static inline void ili9341_send_color(void * data, uint16_t length)
{
    disp_wait_for_pending_transactions();
    gpio_set_level(ILI9341_DC, 1);   /*Data mode*/
