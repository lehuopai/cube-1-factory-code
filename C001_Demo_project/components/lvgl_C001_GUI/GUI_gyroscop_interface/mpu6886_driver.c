...
/**
  ...
  * @retval void
  */
static void MPU6886I2C_Read_NBytes(uint8_t reg_Addr, uint8_t len, uint8_t *read_Buffer)
{
    //I2CreadBuff(&wire0, driver_Addr, start_Addr, read_Buffer, len);
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	ESP_ERROR_CHECK(i2c_master_start(cmd));
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd, (MPU6886_ADDRESS << 1) | WRITE_BIT, 1));
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd, reg_Addr, 1));
	ESP_ERROR_CHECK(i2c_master_stop(cmd));
	ESP_ERROR_CHECK(i2c_master_cmd_begin(MPU6886_I2C_MASTER_NUM, cmd, 1000/portTICK_PERIOD_MS));
    ...
    if(len == 0 )
		return ;
    ESP_ERROR_CHECK(i2c_master_start(cmd));
    ESP_ERROR_CHECK(i2c_master_write_byte(cmd, (MPU6886_ADDRESS << 1) | READ_BIT, ACK_CHECK_EN));
    ESP_ERROR_CHECK(i2c_master_read(cmd, read_Buffer, len, I2C_MASTER_LAST_NACK));
    ESP_ERROR_CHECK(i2c_master_stop(cmd));
    ESP_ERROR_CHECK(i2c_master_cmd_begin(MPU6886_I2C_MASTER_NUM, cmd, 1000 / portTICK_RATE_MS));
    i2c_cmd_link_delete(cmd);
}
...
/**
  ...
  * @retval void
  */
static void MPU6886I2C_Write_NBytes(uint8_t reg_Addr, uint8_t len, uint8_t *write_Buffer)
{
    // I2Cwirtebuff(&wire0, driver_Addr, reg_Addr, write_Buffer, len);
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    ESP_ERROR_CHECK(i2c_master_start(cmd));
    ESP_ERROR_CHECK(i2c_master_write_byte(cmd, (MPU6886_ADDRESS << 1) | WRITE_BIT, ACK_CHECK_EN));
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd, reg_Addr, ACK_CHECK_EN));
    ESP_ERROR_CHECK(i2c_master_write(cmd, write_Buffer, len, ACK_CHECK_EN));
    ESP_ERROR_CHECK(i2c_master_stop(cmd));
    ESP_ERROR_CHECK(i2c_master_cmd_begin(MPU6886_I2C_MASTER_NUM, cmd, 1000 / portTICK_RATE_MS));
    i2c_cmd_link_delete(cmd);
}
...
int MPU6886Init(void)
{
    ...
    regdata = 0x00;
    MPU6886I2C_Write_NBytes(MPU6886_PWR_MGMT_1, 1, &regdata);
    ...
    regdata = 0x01;
    MPU6886I2C_Write_NBytes(MPU6886_CONFIG, 1, &regdata);
    vTaskDelay(10 / portTICK_PERIOD_MS);

    regdata = 0x05;
    MPU6886I2C_Write_NBytes(MPU6886_SMPLRT_DIV, 1, &regdata);
    vTaskDelay(10 / portTICK_PERIOD_MS);

    regdata = 0x00;
    MPU6886I2C_Write_NBytes(MPU6886_INT_ENABLE, 1, &regdata);
    vTaskDelay(10 / portTICK_PERIOD_MS);

    regdata = 0x00;
    MPU6886I2C_Write_NBytes(MPU6886_ACCEL_CONFIG2, 1, &regdata);
    vTaskDelay(10 / portTICK_PERIOD_MS);
    ...
    regdata = 0x00;
    MPU6886I2C_Write_NBytes(MPU6886_FIFO_EN, 1, &regdata);
    vTaskDelay(10 / portTICK_PERIOD_MS);

    regdata = 0x22;
    MPU6886I2C_Write_NBytes(MPU6886_INT_PIN_CFG, 1, &regdata);
    vTaskDelay(10 / portTICK_PERIOD_MS);
